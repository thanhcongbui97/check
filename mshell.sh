	...
	do
		...
		do
			...
			then
                            ...
                            local counter=$(awk -v mvar=$tmp_er '{for ( i=0; i<=NF; i++) if ( $i==mvar ) print $i}' $database|wc -l)
                            local length1=$(echo ${#er1})
                            (( length1-=2 ))
                            local tmp_er1=$(echo ${er1:1:length1})
                            local counter1=$(awk -v mvar=$tmp_er1 '{for ( i=0; i<=NF; i++) if ( $i==mvar ) print $i}' $database|wc -l)
                            local length2=$(echo ${#er2})
                            (( length2-=2 ))
                            local tmp_er2=$(echo ${er2:1:length2})
                            local counter2=$(awk -v mvar=$tmp_er2 '{for ( i=0; i<=NF; i++) if ( $i==mvar ) print $i}' $database|wc -l)
